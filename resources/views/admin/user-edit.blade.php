...
@section('content')
    ...
    <form action="{{ route('user.update', $user) }}" method="POST" class="bg-white p-6 rounded-lg shadow-md">
        @csrf
        @method('PUT')
        <div class="mb-4">
        ...
        <div class="mb-4">
            ...
            <input type="email" id="email" name="email"
                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                placeholder="masukkan email user" value="{{ old('email', $user->email) }}">
            @error('email')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
        <div class="mb-4">
            ...
            <input type="text" id="telp" name="telp"
                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                placeholder="masukkan nomor telp user" value="{{ old('telp', $user->telp) }}">
            @error('telp')
                <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
